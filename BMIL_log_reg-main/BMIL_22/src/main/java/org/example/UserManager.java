...
public class UserManager {
    ...
    public void register() {
        ...
        try { //считаем интервал между нажатиями клавиш при регистрации
            while (true) {
 ...
 */
        ...
        try {
            while (true) {
                if (System.in.available() > 0) { // Проверяем, есть ли символы во входящем потоке
                    ...
                    long currentPressTime = System.nanoTime();
                    if (passwordBuilder.length() > 1) { // Игнорируем интервал перед первым символом
                        typingIntervals.add(currentPressTime - lastPressTime);
                    }
                    lastPressTime = currentPressTime;
                }
    ...
    */
    public void login() {
        ...
        if (userRepository.authenticate(username, password)) {
            ...
            if (compareIntervals(registeredIntervals, typingIntervals)) {
                System.out.println("Вы успешно вошли в систему. Биометрия совпадает.");
            } else {
                System.out.println("Предупреждение: Биометрия не совпадает.");
            }
        } else {
            System.out.println("Ошибка: Неправильное имя пользователя или пароль.");
        }
    ...
    // Метод для сравнения интервалов
    private boolean compareIntervals(List<Long> registeredIntervals, List<Long> inputIntervals) {
        ...
        for (int i = 0; i < registeredIntervals.size(); i++) {
            ...
            if (difference > 50000000) { // Допустимая погрешность (50 мс)
                return false;
            }
        }
        return true;
